...
#############################################
# Base batch transform (shared contract)
#############################################

# Shared ancestor for the two Data transform families:
#   - DataAugmentation: stochastic data perturbation (e.g. add noise at SNR);
#   - DataAdaptation:   dataset-representation -> model-input adaptation
#                       (e.g. waveform -> spectrogram).
#
# Both are dict -> dict batch transforms, so they share this call/validation
# contract. This is a utility/contract base: it defines no default config and is
# not registered, so this folder intentionally has no registry.py / factory.py.
...
class BaseBatchTransform(ABC):
    ...
    A transform maps a batch dict to a batch dict:
