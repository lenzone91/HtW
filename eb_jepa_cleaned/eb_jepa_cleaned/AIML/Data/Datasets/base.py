...
class BaseDataset(Dataset):
    ...
    This is an abstract base: it owns the sample convention and shared
    validation/conversion helpers. It defines no default config and is not
    registered; concrete datasets subclass it and register themselves.
    """
    ...
    def to_tensor(self, data: Any) -> torch.Tensor:
        ...
        """
        return torch.tensor(data, dtype=self.canonical_dtype)
    ...
    def check_required_keys(self, sample: dict[str, Any]) -> None:
        """
        Check that a sample contains all required keys.
        """
        missing_keys = [
            key for key in self.required_sample_keys if key not in sample
        ]
    ...
    def check_sample(self, sample: dict[str, Any]) -> None:
        ...
        if not isinstance(sample["metadata"], dict):
            raise TypeError(
                "Expected sample['metadata'] to be a dict, "
