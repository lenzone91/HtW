...
class AcVideoJepaBlocksBuilder:
    """
    Build AcVideoJepa architecture blocks from a plain dictionary config.
    """
    ...
    def __init__(
        ...
        self.default_config = deepcopy(
            ...
            else DEFAULT_AC_VIDEO_JEPA_BLOCKS_CONFIG
        )
        self.strict = strict
    ...
    def prepare_config(self, config: dict) -> dict:
        if not isinstance(config, dict):
            raise TypeError(
                "AcVideoJepa blocks config must be a dictionary, "
        ...
        self.check_known_keys(user_config, prepared_config, section_name="blocks")
...
class AcVideoJepaBuilder:
    ...
    def __call__(
        ...
        blocks_builder = AcVideoJepaBlocksBuilder(
            default_config=self.extract_blocks_config(prepared_config),
            strict=self.strict,
        )
        blocks = blocks_builder(
            config=self.extract_blocks_config(prepared_config),
        ...
        return AcVideoJepa(
            ...
            aencoder=blocks.action_encoder,
            predictor=blocks.predictor,
    ...
    def extract_blocks_config(self, config: dict) -> dict:
        return {
            key: deepcopy(config[key])
            for key in DEFAULT_AC_VIDEO_JEPA_BLOCKS_CONFIG
        }
...
def build_ac_video_jepa_blocks(
    config: dict | None = None,
    runtime_context: dict | None = None,
    strict: bool = True,
) -> AcVideoJepaBlocks:
    builder = AcVideoJepaBlocksBuilder(strict=strict)
    return builder(
        config=config or DEFAULT_AC_VIDEO_JEPA_BLOCKS_CONFIG,
        runtime_context=runtime_context,
    )
